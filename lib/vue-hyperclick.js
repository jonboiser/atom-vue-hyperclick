...
function getScopesAtWord(textEditor, pos) {
  // entity.name.tag.other.html: tag names
  // entity.other.attribute-name.html: attributes
  return textEditor.scopeDescriptorForBufferPosition(pos).getScopesArray();
}

function isHtmlTagName(textEditor, range) {
  return getScopesAtWord(textEditor, range.start).includes('entity.name.tag.other.html');
}
...
function makeProvider(ctx) {
  ...
  return {
    ...
    getSuggestionForWord(textEditor, text, range) {
      ...
      if (isHtmlTagName(textEditor, range) && text.includes('-')) {
        /* eslint-disable no-param-reassign */
        const camelText = camelcase(text);
        const pascalText = upperFirst(camelText);
        range.end.column -= 1;
      ...
      result = result || provider.getSuggestionForWord(patchedEditor, text, range);
